...
export default {
  ...
  addTag(state, id) {
    ...
    if (menuObject != null) {
      ...
      if (
        Array.isArray(state.cachePages) &&
        (menuObject.onlineFormId == null || menuObject.onlineFormId === '') &&
  ...
  removeTag(state, id) {
    ...
    if (tagItem == null) return;
    // 移除页面缓存
    let cachePages = state.tagList.map(item => item.formRouterName).filter(item => item != null);
    state.cachePages = state.cachePages.filter(item => {
      return cachePages.indexOf(item) !== -1;
    });
    setObjectToSessionStorage('cachePages', state.cachePages);
    setObjectToSessionStorage('tagList', state.tagList);
  ...
  closeOtherTags(state, id) {
    ...
    if (
      menuObject &&
      (menuObject.onlineFormId == null || menuObject.onlineFormId === '') &&
      menuObject.formRouterName &&
      menuObject.formRouterName !== ''
    ) {
      state.cachePages = [menuObject.formRouterName];
      if (setObjectToSessionStorage('currentMenuId', menuObject.menuId))
        state.currentMenuId = menuObject.menuId;
    }
    setObjectToSessionStorage('cachePages', state.cachePages);
    setObjectToSessionStorage('tagList', state.tagList);
  },
    ...
    if (Array.isArray(list)) {
      list.forEach(item => {
        if (item.extraData != null && item.extraData !== '') {
          ...
          item.bindType = item.extraData.bindType || item.bindType;
          item.onlineFormId = item.extraData.onlineFormId || item.onlineFormId;
          item.onlineFlowEntryId = item.extraData.onlineFlowEntryId || item.onlineFlowEntryId;
          item.reportPageId = item.extraData.reportPageId || item.reportPageId;
          item.formRouterName = item.extraData.formRouterName || item.formRouterName;
          item.targetUrl = item.extraData.targetUrl;
        }
      ...
      if (setObjectToSessionStorage('menuList', list))
        state.menuList = treeDataTranslate(list, 'menuId', 'parentId');
    ...
    if (Array.isArray(state.tagList) && Array.isArray(state.menuList)) {
      for (let i = 0; i < state.menuList.length; i++) {
        ...
        if (menuItem != null) {
          ...
          // 添加新缓存
          if (Array.isArray(state.cachePages)) {
            let cachePages = state.tagList
              .map(item => item.formRouterName)
              .filter(item => item != null);
            if (state.cachePages.indexOf(menuItem.formRouterName) === -1) {
              state.cachePages.push(menuItem.formRouterName);
            }
            // 移除二级页面缓存
            state.cachePages = state.cachePages.filter(item => {
              return cachePages.indexOf(item) !== -1;
  ...
  setHeadImage: (state, imageHeaderUrl) => {
    if (state.userInfo) {
      state.userInfo.headImageUrl = imageHeaderUrl;
      setObjectToSessionStorage('userInfo', state.userInfo);
    }
  },
  ...
  setUserShowNameData: (state, data) => {
    state.userShowNameData = data;
  }
};
