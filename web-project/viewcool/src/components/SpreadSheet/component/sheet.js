...
function insertDeleteRowColumn (type) {
  ...
  if (type === 'insert-row') {
    ...
    const { ri, ci } = data.selector;
    this.trigger('cell-edited', '', ri, ci);
...
async function uploadImage(type) {
  try {
    ...
    reader.onload =  function (evt) {
      ...
      // data.setSelectedCellAttr('value', imgUrlBase64); // 设置图片地址。方面后面使用地址渲染。
      // data.setSelectedCellAttr('left',  data.getSelectedRect().left);
      // data.setSelectedCellAttr('top',  data.getSelectedRect().top);
      data.rows._[data.selector.ri].cells[data.selector.ci].value = imgUrlBase64
      data.rows._[data.selector.ri].cells[data.selector.ci].left = data.getSelectedRect().left
      data.rows._[data.selector.ri].cells[data.selector.ci].top = data.getSelectedRect().top
      const image = new Image();
      image.src = imgUrlBase64;
      image.onload = function() {
        const width = this.width;
        const height = this.height;
        // data.setSelectedCellAttr('imagewidth', width);
        // data.setSelectedCellAttr('imageheight', height);
        data.rows._[data.selector.ri].cells[data.selector.ci].imagewidth = width
        data.rows._[data.selector.ri].cells[data.selector.ci].imageheight = height
      };

      sheetReset.call(that);
    ...
    // const imageUrl = 'C:\Users\shm\Pictures\29-20040Q60Z70-L.jpg'
    // data.setSelectedCellAttr('type', 'image' || type); // 设置类型，方便后面的渲染。
    // data.setSelectedCellAttr('value', imgUrlBase64); // 设置图片地址。方面后面使用地址渲染。
    // sheetReset.call(this);
  } catch (e) {
...
export default class Sheet {
  constructor (targetEl, data) {
    ...
    this.toolbar = new Toolbar(data,getWidth, !showToolbar);
