...
function renderSelectedHeaderCell (x, y, w, h) {
  const { draw } = this;
  draw.save();
  draw.attr({ fillStyle: 'rgba(76,76,76,.1)' })
    .fillRect(x, y, w, h);
  draw.restore();
}
function renderLeftHeaderCell (x, y, w, h, op) {
  ...
  op = op || {}
  draw.save();
  draw.attr({ fillStyle: 'rgba(76,76,76,.1)', ...op})
    .fillRect(x, y, w, h);
  draw.restore();
}
// viewRange
...
function renderFixedHeaders (type, viewRange, w, h, tx, ty) {
  ...
  if (type === 'all' || type === 'left') {
    data.rowEach(viewRange.sri, viewRange.eri, (i, y1, rowHeight) => {
      ...
      const ii = i;
      draw.line([0, y], [w, y]);
      if (data.settings.leftFixHeaderRender && data.settings.leftFixHeaderRender instanceof Function) {
