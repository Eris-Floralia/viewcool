  ...
  methods: {
    ...
    emitChange() {
      ...
      if (this.multiple) {
        ...
        dictData = {
          id: tempValue,
          name: (this.selectedItems[0] || {})[this.props.label]
        };
      }
...
<style scoped>
.dept-select ::v-deep .dept-select-popper {
  display: none;
}

.dept-select ::v-deep .el-dialog__header {
  height: 42px;
  line-height: 42px;
  padding: 0px 20px;
  background-color: #f8f8f8;
}
.dept-select ::v-deep .el-dialog__title {
  color: #333;
  font-size: 14px;
}
.dept-select ::v-deep .el-dialog__headerbtn {
  top: 12px;
}
.dept-select ::v-deep .el-dialog__body {
  padding: 25px;
}
</style>
