<template>
  ...
  >
    <el-scrollbar :style="{ height: this.height, 'min-width': this.width }" ref="scrollbar">
      ...
      >
        <span :style="getNodeStyle(data)" slot-scope="{ data }">{{
          data[getDataProps.label]
        }}</span>
      </el-tree>
    </el-scrollbar>
    <el-select
    ...
    >
      <el-option
        ...
        :key="item[getDataProps.value]"
        :value="item[getDataProps.value]"
        :label="item[getDataProps.label]"
      />
    </el-select>
  </el-popover>
</template>
  ...
  methods: {
    ...
    parseNode(node) {
      if (Array.isArray(node) && node.length > 0) {
        node.forEach(item => {
          item['__node_is_leaf__'] = !(
            Array.isArray(item[this.getDataProps.children]) &&
            item[this.getDataProps.children].length > 0
          );
        });
        return node;
      } else {
    ...
    getNodeStyle(data) {
      if (
        !this.multiple &&
        (this.selectNodes[0] || {})[this.getDataProps.value] === data[this.getDataProps.value]
      ) {
        return {
          ...
          if (Array.isArray(this.value)) {
            this.value.forEach(item => {
              let data = findTreeNode(
                this.allTreeNode,
                item,
                this.getDataProps.value,
                this.getDataProps.children
              );
    ...
    value: {
      handler(newValue) {
        ...
        if (this.$refs.dropdownTree) {
          this.multiple
            ? this.$refs.dropdownTree.setCheckedKeys(newValue)
            : this.$refs.dropdownTree.setCurrentKey(newValue);
        }
      },
      immediate: true
    }
