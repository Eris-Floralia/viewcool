        ...
        <el-col :span="12">
          <el-form-item label="数据权限名称" prop="SysDataPerm.dataPermName">
            <el-input class="input-item" v-model="formData.SysDataPerm.dataPermName"
              :clearable="true" placeholder="显示名称" maxlength="30" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          ...
          <el-tabs class="dataperm-data" type="border-card">
              ...
              <el-row>
                ...
                <el-col :span="24">
                  <el-scrollbar style="height: 279px;" wrap-class="scrollbar_dropdown__wrap">
                    <div class="table-empty unified-font" v-if="deptTree.length <= 0" style="margin-top: 35px;">
                      <img src="@/assets/img/empty.png">
                      <span>暂无数据</span>
                    </div>
                    <el-tree v-show="deptTree.length > 0" ref="deptTree" :data="deptTree" show-checkbox node-key="id" default-expand-all
            ...
            <el-tab-pane label="菜单列表">
              <el-scrollbar style="height: 330px;" wrap-class="scrollbar_dropdown__wrap">
                <div class="table-empty unified-font" v-if="menuTree.length <= 0" style="margin-top: 50px;">
                  <img src="@/assets/img/empty.png">
                  <span>暂无数据</span>
                </div>
                <el-tree v-show="menuTree.length > 0" ref="menuTree" :data="menuTree" show-checkbox node-key="id" default-expand-all
                  :check-strictly="false" :props="{...menuProps}" :filter-node-method="filterDeptNode" />
              </el-scrollbar>
            </el-tab-pane>
          </el-tabs>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    loadMenuList () {
      return new Promise((resolve, reject) => {
        let params = {};
        DictionaryController.dictSysMenu(this, params).then(res => {
          this.menuTree = treeDataTranslate(res.getList(), 'id');
          if (Array.isArray(this.formData.SysDataPerm.dataPermMenuList)) {
            this.$refs.menuTree.setCheckedKeys(this.formData.SysDataPerm.dataPermMenuList.map((item) => {
              return item.menuId;
            }));
          }
          resolve();
...
<style scoped>
  ...
  .dataperm-data ::v-deep .el-tabs__header {
    margin-bottom: 0px;
  }
  .dataperm-data ::v-deep .el-tabs__content {
    height: 360px;
    overflow: hidden;
  }
</style>
