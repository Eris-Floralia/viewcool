...
export default {
  ...
  methods: {
    ...
    loadAuthData() {
      DictionaryController.dictAllMenu(this, {})
        .then(res => {
          let menuList = res.getList();
          this.authData = treeDataTranslate(menuList, 'id', 'parentId');
          if (Array.isArray(this.formData.menuIdList)) {
            let tempList = [];
            this.formData.menuIdList.forEach(item => {
              let tempMenu = findItemFromList(menuList, item, 'id');
              // 判断是否为叶子节点
              if (
                tempMenu != null &&
