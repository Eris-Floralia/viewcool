  ...
  methods: {
    ...
    onSaveClick() {
      this.$refs.fragmentEditPost.validate(valid => {
        if (!valid) return;
        if (this.formData.SysPost.postName == null || this.formData.SysPost.postLevel == null) {
        ...
        let params = {
          sysPostDto: {
            ...
            postLevel: this.formData.SysPost.postLevel,
            leaderPost: this.formData.SysPost.leaderPost
          }
        };

        let httpCall = this.isEdit
          ? SysPostController.update(this, params)
          : SysPostController.add(this, params);
        httpCall
          .then(res => {
    ...
    loadSysPostData() {
      return new Promise((resolve, reject) => {
        if (!this.formData.SysPost.isDatasourceInit && this.isEdit) {
          ...
          SysPostController.view(this, params)
            .then(res => {
