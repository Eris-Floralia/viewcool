    ...
    <el-row :gutter="20" class="full-width-input">
      <el-col :span="24">
        <el-form-item label="登录名称" prop="loginName">
          <el-input v-model="formData.loginName" placeholder="用户登录名称" clearable :disabled="isEdit" maxlength="30" />
        </el-form-item>
        <el-form-item label="登录密码" v-if="!isEdit" prop="password">
          <el-input v-model="formData.password" type="password" placeholder="用户登录密码" clearable maxlength="64" />
        </el-form-item>
        <el-form-item label="确认密码" v-if="!isEdit" prop="passwordRepeat">
          <el-input v-model="formData.passwordRepeat" type="password" placeholder="再次输入用户密码" clearable maxlength="64" />
        </el-form-item>
        ...
        <el-form-item label="用户状态" prop="userStatus" v-if="isEdit">
          <el-radio-group v-model="formData.userStatus">
            <el-radio v-for="item in SysUserStatus.getList()" :key="item.id" :label="item.id">{{item.name}}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="所属部门" prop="deptId">
          <el-cascader class="input-item" v-model="deptId.value" :clearable="true"
            placeholder="所属部门" :loading="deptId.impl.loading" :props="{ value: 'id', label: 'name', checkStrictly: true }"
...
export default {
  ...
  data () {
    return {
      ...
      deptId: {
        impl: new DropdownWidget(this.loadDeptmentDropdownList, true, 'id'),
        value: []
      },
      rules: {
  ...
  methods: {
    ...
    loadRole() {
      DictionaryController.dictSysRole(this, {})
        .then(res => {
    ...
    loadDataPerm() {
      DictionaryController.dictSysDataPerm(this, {})
        .then(res => {
          this.dataPermList = res.getList();
        })
        .catch(e => {});
    },
    loadRowData (id) {
  ...
  mounted () {
    ...
    this.deptId.impl.onVisibleChange(true).then(res => {
      this.deptId.value = findTreeNodePath(this.deptId.impl.dropdownList, this.formData.deptId, 'id');
    });
    this.loadRole();
    this.loadDataPerm();
    this.loadDeptPost();
