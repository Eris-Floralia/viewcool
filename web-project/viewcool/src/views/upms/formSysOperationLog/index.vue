    ...
    >
      <filter-box :item-width="350" @search="refreshFormOperationType(true)" @reset="onReset">
        ...
        <el-form-item label="调用时长" prop="formFilter.elapse">
          <input-number-range
            class="filter-item"
            v-model="formSysOperationLog.formFilter.elapse"
            :clearable="true"
          />
        </el-form-item>
        <el-form-item label="操作日期" prop="formFilter.operationTime">
          <date-range
            class="filter-item"
            v-model="formSysOperationLog.formFilter.operationTime"
            :clearable="true"
            :allowTypes="['day']"
            align="left"
            range-separator="-"
      ...
      formSysOperationLog: {
        ...
        operationLog: {
          impl: new TableWidget(
            this.loadOperationLogWidgetData,
            this.loadOperationLogVerify,
            true,
            false,
            'logId',
            false
    ...
    loadOperationLogWidgetData(params) {
      ...
      params = {
        ...
        sysOperationLogDtoFilter: {
          ...
          elapseMin: Array.isArray(this.formSysOperationLog.formFilterCopy.elapse)
            ? this.formSysOperationLog.formFilterCopy.elapse[0]
            : undefined,
          elapseMax: Array.isArray(this.formSysOperationLog.formFilterCopy.elapse)
            ? this.formSysOperationLog.formFilterCopy.elapse[1]
            : undefined,
          operationTimeStart: Array.isArray(this.formSysOperationLog.formFilterCopy.operationTime)
            ? this.formSysOperationLog.formFilterCopy.operationTime[0]
            : undefined,
          operationTimeEnd: Array.isArray(this.formSysOperationLog.formFilterCopy.operationTime)
            ? this.formSysOperationLog.formFilterCopy.operationTime[1]
            : undefined
        }
      };
      return new Promise((resolve, reject) => {
        SystemController.listSysOperationLog(this, params)
          .then(res => {
            resolve({
