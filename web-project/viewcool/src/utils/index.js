...
export function stringCase(str, type) {
 ...
 */
export function findNode(node, id, list, idKey = 'id', childKey = 'children') {
...
export function deepMerge(obj1, obj2) {
  let tempObj = copyObject(obj1);
  if (obj2 != null) {
    Object.keys(obj2).forEach(key => {
      let val2 = obj2[key];
      let val1 = tempObj[key];
      if (isObject(val2)) {
        // 如果两个值都是对象，则递归合并
        if (isObject(val1)) {
