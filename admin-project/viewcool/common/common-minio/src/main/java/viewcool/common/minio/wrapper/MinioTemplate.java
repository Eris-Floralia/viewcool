package viewcool.common.minio.wrapper;

import viewcool.common.core.exception.MyRuntimeException;
import viewcool.common.minio.config.MinioProperties;
import io.minio.*;
    ...
    public void putObject(String bucketName, String objectName, InputStream stream) {
        try {
            client.putObject(PutObjectArgs.builder()
                    .bucket(bucketName).object(objectName).stream(stream, -1, 5242880).build());
        } catch (Exception e) {
            throw new MyRuntimeException(e);
        } finally {
            try {
