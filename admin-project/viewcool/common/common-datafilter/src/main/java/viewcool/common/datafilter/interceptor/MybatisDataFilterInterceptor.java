package viewcool.common.datafilter.interceptor;
...
import viewcool.common.datafilter.config.DataFilterProperties;
import lombok.Data;
...
public class MybatisDataFilterInterceptor implements Interceptor {
    ...
    public void loadInfoWithDataFilter() {
        ...
        for (BaseDaoMapper<?> mapperProxy : mapperMap.values()) {
            ...
            if (mapperClass == null) {
                try {
                    mapperProxy = (BaseDaoMapper<?>)
                            ((Advised) ReflectUtil.getFieldValue(proxy, "advised")).getTargetSource().getTarget();
                    proxy = ReflectUtil.getFieldValue(mapperProxy, "h");
                    mapperClass = (Class<?>) ReflectUtil.getFieldValue(proxy, "mapperInterface");
                } catch (Exception e) {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (GlobalThreadLocal.enabledTenantIdFilter()
                &&  BooleanUtil.isTrue(properties.getEnabledTenantFilter())) {
    ...
    private JSONObject getAndVerifyMenuDataPerm(JSONObject allMenuDataPermMap, String sqlId) {
        ...
        if (BooleanUtil.isTrue(properties.getEnableMenuPermVerify())) {
            ...
            Assert.notNull(url);
            if (!this.verifyMenuPerm(null, url, sqlId) && !this.verifyMenuPerm(menuId, url, sqlId)) {
