...
public class RedisKeyUtil {
     ...
     * @return session缓存的键前缀。
     */
    public static String getSessionIdPrefix(TokenData tokenData, String loginName) {
        if (tokenData.getTenantId() == null) {
            return SESSIONID_PREFIX + loginName + "_";
        }
        return SESSIONID_PREFIX + tokenData.getTenantId() + "_" + loginName + "_";
    ...
    /**
     * 计算SessionId关联的权限字存储于Redis中的键。
     *
     * @param sessionId 会话Id。
     * @return 会话关联的权限字存储于Redis中的键值。
     */
    public static String makeSessionPermCodeKey(String sessionId) {
        return "PERM_CODE:" + sessionId;
    }
