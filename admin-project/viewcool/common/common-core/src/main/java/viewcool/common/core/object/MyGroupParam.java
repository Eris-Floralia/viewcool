...
import viewcool.common.core.exception.InvalidClassFieldException;
import viewcool.common.core.exception.InvalidDataFieldException;
import viewcool.common.core.exception.InvalidDataModelException;
...
public class MyGroupParam extends ArrayList<MyGroupParam.GroupInfo> {
    ...
    private void processGroupInfo(
        ...
        if (coreProperties.isMySql() || coreProperties.isDm()) {
            ...
            this.processSqlServerGroupInfoWithDateAggregation(groupInfo, baseData, groupBy, groupSelect);
        } else {
    ...
    private void processSqlServerGroupInfoWithDateAggregation(
            GroupInfo groupInfo, GroupBaseData baseData, StringBuilder groupBy, StringBuilder groupSelect) {
        if (ApplicationConstant.DAY_AGGREGATION.equals(groupInfo.dateAggregateBy)) {
            groupBy.append("CONVERT(VARCHAR, ")
                    .append(baseData.tableName).append(".").append(baseData.columnName);
            groupSelect.append("CONVERT(VARCHAR, ")
                    .append(baseData.tableName).append(".").append(baseData.columnName);
            groupBy.append(", 23)");
            groupSelect.append(", 23)");
        } else if (ApplicationConstant.MONTH_AGGREGATION.equals(groupInfo.dateAggregateBy)) {
            groupBy.append("CONVERT(VARCHAR(7), ")
                    .append(baseData.tableName).append(".").append(baseData.columnName);
            groupSelect.append("CONVERT(VARCHAR(7), ")
                    .append(baseData.tableName).append(".").append(baseData.columnName);
            groupBy.append(", 23) + '-01'");
            groupSelect.append(", 23) + '-01'");
        } else if (ApplicationConstant.YEAR_AGGREGATION.equals(groupInfo.dateAggregateBy)) {
            groupBy.append("CONVERT(VARCHAR(4), ")
                    .append(baseData.tableName).append(".").append(baseData.columnName);
            groupSelect.append("CONVERT(VARCHAR(4), ")
                    .append(baseData.tableName).append(".").append(baseData.columnName);
            groupBy.append(", 23) + '-01-01'");
            groupSelect.append(", 23) + '-01-01'");
        } else {
            throw new IllegalArgumentException("Illegal DATE_FORMAT for GROUP ID list.");
        }
    }
