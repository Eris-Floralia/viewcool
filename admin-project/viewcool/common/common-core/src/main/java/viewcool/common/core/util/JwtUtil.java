...
public class JwtUtil {
    ...
    public static String generateToken(Map<String, Object> claims, long expirationMillisecond, String signingKey) {
        ...
        SecretKey sk = Keys.hmacShaKeyFor(signingKey.getBytes());
        String token = Jwts.builder().claims(claims)
                .signWith(sk, Jwts.SIG.HS256)
                .expiration(new Date(createTime + expirationMillisecond))
                .compact();
    ...
    public static Claims parseToken(String token, String signingKey) {
        ...
        try {
            SecretKey sk = Keys.hmacShaKeyFor(signingKey.getBytes());
            claims = Jwts.parser().verifyWith(sk).build().parseSignedClaims(tokenKey).getPayload();
        } catch (Exception e) {
