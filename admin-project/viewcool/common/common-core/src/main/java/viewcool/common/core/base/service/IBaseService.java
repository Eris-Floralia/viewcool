...
import viewcool.common.core.object.TableModelInfo;
...
public interface IBaseService<M, K extends Serializable> extends IService<M> {
     ...
     * 判断参数值主键集合中的所有数据，是否全部存在
     *
     * @param ids 逗号分隔的待校验的主键集合。
     * @return 全部存在返回true，否则false。
     */
    boolean existAllPrimaryKeys(String ids);
     ...
     *
     * @param inFilterField  待校验的数据字段，这里使用Java对象中的属性，如courseId，而不是数据字段名course_id
     * @param inFilterValues 逗号分隔的过滤值字符串。
     * @param clazz          过滤值的字段类型。
     * @return 全部存在返回true，否则false。
     */
    <T> boolean existUniqueKeyList(String inFilterField, String inFilterValues, Class<T> clazz);
    ...
    /**
     * 指定字段的唯一值验证。验证失败会抛出DuplicateKeyException异常，同时给出详细的字段重复信息。
     *
     * @param data         新数据对象。
     * @param originalData 原有数据对象。
     * @return 应答结果对象。
     */
    CallResult verifyUniqueFieldValue(M data, M originalData);
